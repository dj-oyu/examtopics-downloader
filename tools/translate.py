...
def fetch_thread(conn: sqlite3.Connection, tid: int) -> dict | None:
    ...
    messages = conn.execute(
        "SELECT id, role, author, content, created_at FROM explanation_messages "
...
def cmd_reply(conn: sqlite3.Connection, args) -> None:
    ...
    Append an agent message to a thread. stdin payload:
      {
        ...
        "resolve": true,                   // optional, also marks thread resolved
        "explanation_ja": "..."            // optional, also rewrites questions.explanation_ja
      }
    """
    ...
    if not content and not payload.get("explanation_ja"):
        sys.exit("reply: payload must include 'content' or 'explanation_ja'")
    ...
    print(
        json.dumps(
            {"thread_id": tid, "question_id": t["question_id"], "actions": actions},
            ensure_ascii=False,
        )
    )
...
def main() -> int:
    ...
    p_reply = sub.add_parser(
        "reply",
        help="read JSON {content, author?, resolve?, explanation_ja?} and append "
        "an agent message to the thread",
    )
