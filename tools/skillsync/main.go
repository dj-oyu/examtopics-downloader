...
//       - .gemini/skills/exam-translator/SKILL.md (discovered by
//         gemini-cli for the orchestrator dispatch).
//
//   agents/exam-translator-worker.md  →  the dedicated worker
//     subagent definition that the orchestrator delegates each
//     literal `bulk-next` invocation to. Gemini-only, so it has a
//     single target:
//       - .gemini/agents/exam-translator-worker.md
//
// Wiring:
//
	...
	jobs := []syncJob{
		{
			...
			targets: []string{
				...
				filepath.Join(root, ".gemini", "skills", "exam-translator", "SKILL.md"),
			},
		},
		{
			src: filepath.Join(root, "agents", "exam-translator-worker.md"),
			targets: []string{
				filepath.Join(root, ".gemini", "agents", "exam-translator-worker.md"),
			},
		},
	}
	for _, j := range jobs {
